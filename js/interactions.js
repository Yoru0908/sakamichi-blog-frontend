...
const InteractionPage = {
    ...
    if (!data) {
      area.innerHTML = `
        <div class="empty-state">
          ...
          <h3 style="font-size: 16px; font-weight: 500; color: #374151; margin-bottom: 8px;">暂无数据</h3>
          <p>该月份暂无互动分析数据</p>
        </div>
      `;
      return;
      ...
      return `
        <tr>
          ...
          <td style="text-align: right; white-space: nowrap;">
            <span style="font-weight: 700; color: #7e57c2;">${m.total}次</span>
          </td>
        </tr>
      `;
    }).join('');
  ...
  renderRanking(data, area) {
    ...
    area.innerHTML = `
      <div class="panel">
        ...
        <p style="font-size: 13px; color: #9ca3af; margin-bottom: 20px;">谁是本月的"话题中心"——被其他成员在博客中提到最多的人</p>
        ${items}
      </div>
    `;
  },
  ...
  renderGeneration(data, area) {
    ...
    function getCellColor(val) {
      ...
      if (ratio > 0.7) return { bg: '#7e57c2', text: 'white' };
      if (ratio > 0.4) return { bg: '#b39ddb', text: 'white' };
      if (ratio > 0.2) return { bg: '#e1bee7', text: '#7e57c2' };
      return { bg: '#f3e5f5', text: '#7e57c2' };
    }

    // 热力图 HTML
    let heatmapHTML = `<div class="heatmap-grid" style="grid-template-columns: 80px repeat(${gm.generations.length}, 1fr);">`;
    heatmapHTML += `<div class="heatmap-header" style="font-size: 11px; color: #9ca3af;">提及 → 被提及</div>`;
        ...
        mentionItems = detail.topMentioned.map((m, i) => {
          ...
          return `
            <div class="gen-detail-item">
              <span style="width: 20px; text-align: right; color: #9ca3af; font-size: 13px;">${i + 1}.</span>
              <span style="font-weight: 500; min-width: 80px;">${m.name}</span>
              ${badge}
              <div style="flex: 1; margin: 0 12px; height: 6px; background: #f3e5f5; border-radius: 3px; overflow: hidden;">
                <div style="height: 100%; width: ${pct}%; background: #7e57c2; border-radius: 3px;"></div>
              </div>
              <span style="font-weight: 600; color: #7e57c2; white-space: nowrap;">${m.count}次</span>
            </div>
          `;
        }).join('');
      }
        ...
        receivedItems = received.members.map(m => {
          ...
          return `
            <div class="gen-detail-item">
              <span style="font-weight: 500; min-width: 80px;">${m.name}</span>
              <div style="flex: 1; margin: 0 12px; height: 6px; background: #f3e5f5; border-radius: 3px; overflow: hidden;">
                <div style="height: 100%; width: ${pct}%; background: #ce93d8; border-radius: 3px;"></div>
              </div>
              <span style="font-weight: 600; color: #7e57c2; white-space: nowrap;">${m.count}次</span>
              <span style="color: #9ca3af; font-size: 12px; white-space: nowrap; margin-left: 8px;">← ${m.mainSource} ${m.mainCount}</span>
            </div>
          `;
        }).join('');
      }
    ...
    area.innerHTML = `
      <div class="panel">
        ...
        <p style="font-size: 13px; color: #9ca3af; margin-bottom: 16px;">期别间的提及热力图——数字越大颜色越深</p>
        ${heatmapHTML}
      </div>
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (!dataLoaded && area) {
    area.innerHTML = `
      <div class="empty-state">
        <h3 style="font-size: 16px; font-weight: 500; color: #374151; margin-bottom: 8px;">数据加载失败</h3>
        <p>请通过 HTTP 服务器访问本页面（不支持 file:// 协议）</p>
        <p style="margin-top: 8px; font-size: 12px; color: #d1d5db;">例如: http://localhost:8899/interactions.html</p>
      </div>`;
    return;
  }
